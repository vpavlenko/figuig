...
export default function HomeClient({ showSolution }: { showSolution: boolean }) {
  ...
  function isSourceHighlighted(sentenceId: number, tokenKey: string, tokenIndex: number) {
    ...
    if (activeOrigin === "source") return activeSource === tokenKey;
    if (activeOrigin !== "english") return false;
    ...
    return (
      <div className="entry-source">
        {tokens.map((token, index) => {
          ...
          const displayToken = script === "latin" ? transliterateCyrillicToLatin(token) : token;
          const separator =
            index === 0
              ? ""
              : token === "?"
          ...
          return (
            <span key={`${token}-${index}`}>
              ...
              <span
                className={`token${isSourceHighlighted(sentenceId, tokenKey, index) ? " is-highlighted" : ""}`}
                style={
        ...
        >
          <button
            ...
            style={{
              ...
              boxShadow: script === "latin" ? "rgba(229, 231, 235, 0.25) 0px 0px 0px 1px" : undefined,
            }}
          >
            Latin
          </button>
          <button
            ...
            style={{
              ...
              transition: "background 120ms, color 120ms",
              boxShadow:
                script === "cyrillic" ? "rgba(229, 231, 235, 0.25) 0px 0px 0px 1px" : undefined,
            }}
          >
            Cyrillic
          </button>
              ...
              {column.groups.map((group) => (
                <section key={group.title} className="group">
                  ...
                  <ol className="group-list">
                    {(mode === "solution" ? [...group.sentences, ...(group.solutionAppend ?? [])] : group.sentences).map(
                      (id) => {
                        ...
                        return (
                          <li key={entry.id} className="sentence-item">
                            <div className={`entry-number${isSolutionNumber ? " is-solution" : ""}`}>{entry.id}.</div>
                            <div>
